...
object KinesisWordProducerASL {
  def main(args: Array[String]): Unit = {
    ...
    val totals = generate(streamName, endpointUrl, recordsPerSecond, wordsPerRecord)

    println("Totals for the words sent")
    totals.foreach(println(_))
  }

  def generate(stream: String, endpoint: String, recordsPerSecond: Int, wordsPerRecord: Int): Seq[(String, Int)] = {
